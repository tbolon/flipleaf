...
namespace FlipLeaf.Core.Text.ScrewturnWiki
{
    ...
    public static class Formatter
    {
        ...
        private static readonly Regex DoubleSquareLinkRegex = new Regex(@"(\[\[.+?\]\])", RegexOptions.Compiled);
        ...
        public static string Format(string raw, bool forIndexing, FormattingContext context, PageInfo current, out string[] linkedPages, bool bareBones)
        {
            ...
            match = HardLineBreak.Match(sb.ToString());
            while (match.Success)
            {
                if (!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                {
                ...
                ComputeNoWiki(sb.ToString(), ref noWikiBegin, ref noWikiEnd);
                match = HardLineBreak.Match(sb.ToString(), end);
            }
            ...
            // Links and images
            match = CurrentLinkRegex.Match(sb.ToString());
            while (match.Success)
            {
                if (IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                {
        ...
        private static string BuildToc(List<HPosition> hPos)
        {
            ...
            for (int i = 0; i < hPos.Count; i++)
            {
                ...
                // Indent
                for (int j = 1; j < tocEntry.Level; j++)
                {
                    sb.Append("&nbsp;&nbsp;&nbsp;");
                }
